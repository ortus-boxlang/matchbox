...
pub fn directory_exists(vm: &mut dyn BxVM, args: &[BxValue]) -> Result<BxValue, String> {
    if args.is_empty() { return Err("directoryExists() expects 1 argument".to_string()); }
...
pub fn directory_create(vm: &mut dyn BxVM, args: &[BxValue]) -> Result<BxValue, String> {
    if args.is_empty() { return Err("directoryCreate() expects at least 1 argument".to_string()); }
    let path_str = vm.to_string(args[0]);
    let path = Path::new(&path_str);
...
pub fn directory_delete(vm: &mut dyn BxVM, args: &[BxValue]) -> Result<BxValue, String> {
    if args.is_empty() { return Err("directoryDelete() expects at least 1 argument".to_string()); }
    let path_str = vm.to_string(args[0]);
    let path = Path::new(&path_str);
...
pub fn directory_list(vm: &mut dyn BxVM, args: &[BxValue]) -> Result<BxValue, String> {
    if args.is_empty() { return Err("directoryList() expects at least 1 argument".to_string()); }
    let path_str = vm.to_string(args[0]);
    let recurse = if args.len() > 1 { args[1].as_bool() } else { false };
...
pub fn file_exists(vm: &mut dyn BxVM, args: &[BxValue]) -> Result<BxValue, String> {
    if args.is_empty() { return Err("fileExists() expects 1 argument".to_string()); }
    let path_str = vm.to_string(args[0]);
    let path = Path::new(&path_str);
...
pub fn file_delete(vm: &mut dyn BxVM, args: &[BxValue]) -> Result<BxValue, String> {
    if args.is_empty() { return Err("fileDelete() expects 1 argument".to_string()); }
    let path_str = vm.to_string(args[0]);
...
pub fn file_move(vm: &mut dyn BxVM, args: &[BxValue]) -> Result<BxValue, String> {
    if args.len() < 2 { return Err("fileMove() expects 2 arguments: (source, destination)".to_string()); }
...
pub fn file_copy(vm: &mut dyn BxVM, args: &[BxValue]) -> Result<BxValue, String> {
    if args.len() < 2 { return Err("fileCopy() expects 2 arguments: (source, destination)".to_string()); }
    let src = vm.to_string(args[0]);
    let dest = vm.to_string(args[1]);
...
pub fn file_info(vm: &mut dyn BxVM, args: &[BxValue]) -> Result<BxValue, String> {
    if args.is_empty() { return Err("fileInfo() expects 1 argument".to_string()); }
    let path_str = vm.to_string(args[0]);
    let path = Path::new(&path_str);
    ...
    vm.struct_set(struct_id, "size", BxValue::new_number(metadata.len() as f64));
    vm.struct_set(struct_id, "is_directory", BxValue::new_bool(metadata.is_dir()));
    vm.struct_set(struct_id, "is_file", BxValue::new_bool(metadata.is_file()));
    vm.struct_set(struct_id, "is_symlink", BxValue::new_bool(metadata.file_type().is_symlink()));
    vm.struct_set(struct_id, "is_readonly", BxValue::new_bool(metadata.permissions().readonly()));
...
pub fn file_create_symlink(vm: &mut dyn BxVM, args: &[BxValue]) -> Result<BxValue, String> {
    if args.len() < 2 { return Err("fileCreateSymlink() expects 2 arguments: (link, target)".to_string()); }
...
pub fn file_set_executable(vm: &mut dyn BxVM, args: &[BxValue]) -> Result<BxValue, String> {
    if args.is_empty() { return Err("fileSetExecutable() expects 1 argument".to_string()); }
    let path_str = vm.to_string(args[0]);
    
    #[cfg(unix)]
    {
        use std::os::unix::fs::PermissionsExt;
        let mut perms = fs::metadata(&path_str).map_err(|e| e.to_string())?.permissions();
...
pub fn file_read(vm: &mut dyn BxVM, args: &[BxValue]) -> Result<BxValue, String> {
    if args.is_empty() { return Err("fileRead() expects 1 argument".to_string()); }
    let path_str = vm.to_string(args[0]);
...
pub fn file_write(vm: &mut dyn BxVM, args: &[BxValue]) -> Result<BxValue, String> {
    if args.len() < 2 { return Err("fileWrite() expects 2 arguments: (path, content)".to_string()); }
    let path_str = vm.to_string(args[0]);
