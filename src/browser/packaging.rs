...
pub fn produce_js_bundle(
    ...
    let wasm_bindgen_bin =
        std_env::var("WASM_BINDGEN").unwrap_or_else(|_| "wasm-bindgen".to_string());
...
pub fn produce_fusion_js_bundle(
    ...
    let bootstrap =
        bootstrap::render_fusion_js_bootstrap(&bootstrap::exported_function_names(ast), &stem);
    fs::write(
        &generated_js_path,
        format!("{}\n{}", generated_js, bootstrap),
    )?;
